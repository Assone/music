...
describe('SourceHead.vue', () => {
  it('created', () => {
    const wrapper = shallowMount(SourceHead, {
      propsData: options,
    });

    expect(wrapper.classes()).toContain('source-head');
  });
  ...
  it('render slot', () => {
    ...
    expect(wrapper.findComponent({ name: 'SourceHeadMeta' }).html()).toContain(defaultTemplate);
  });
});
