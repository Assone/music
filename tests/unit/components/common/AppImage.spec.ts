import { shallowMount } from '@vue/test-utils';
import AppImage from '@/components/common/AppImage.vue';

const src =
  'data:image/png;base64,';
const fail = 'data:image/png;base64,fail';

describe('AppImage.vue', () => {
  it('created', () => {
    const wrapper = shallowMount(AppImage, { propsData: { src } });

    expect(wrapper.classes()).toContain('app-image');
  });

  describe('Attributes', () => {
    it('set fit', async () => {
      const wrapper = shallowMount<AppImage & { loading: boolean }>(AppImage, {
        propsData: { src, fit: 'cover' },
      });

      wrapper.vm.loading = false;
      wrapper.vm.$nextTick(() => {
        expect(wrapper.find('.app-image__inner').attributes('style')).toContain(
          'object-fit: cover;',
        );
      });
    });

    it('set src', () => {
      const wrapper = shallowMount<AppImage & { loading: boolean }>(AppImage, {
        propsData: { src },
      });

      wrapper.vm.loading = false;
      wrapper.vm.$nextTick(() => {
        expect(wrapper.find('.app-image__inner').attributes('src')).toBe(src);
      });
    });

    it('set alt', () => {
      const alt = 'alt';
      const wrapper = shallowMount<AppImage & { loading: boolean }>(AppImage, {
        propsData: { src, alt },
      });

      wrapper.vm.loading = false;
      wrapper.vm.$nextTick(() => {
        expect(wrapper.find('.app-image__inner').attributes('alt')).toBe(alt);
      });
    });
  });

  describe('Slot', () => {
    it('render loading', () => {
      const template = '<span>loading</span>';
      const wrapper = shallowMount(AppImage, { propsData: { src }, slots: { loading: template } });

      expect(wrapper.html()).toContain(template);
    });

    it('render error', () => {
      const template = '<span>error</span>';
      const wrapper = shallowMount<AppImage & { loading: boolean; error: boolean }>(AppImage, {
        propsData: { src: fail },
        slots: { error: template },
      });

      wrapper.vm.loading = false;
      wrapper.vm.error = true;

      wrapper.vm.$nextTick(() => {
        expect(wrapper.html()).toContain(template);
      });
    });
  });

  // TODO: test event hanleError
  // TODO: test method handleLoad
  // TODO: test watch updatePath
  // TODO: test method getImageStyle
  // TODO: test liftTime mounted
});
