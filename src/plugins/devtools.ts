...
export function makeLocalGetters(store, namespace) {
  if (!store._makeLocalGettersCache[namespace]) {
    ...
    Object.keys(
      namespace !== 'root' ? store.getters : store._modules.root._rawModule.getters,
    ).forEach((type) => {
      console.log(type);
      // skip if the target getter is not match this namespace
      if (namespace !== 'root') if (type.slice(0, splitPos) !== namespace) return;
...
function formatStoreForInspectorTree(module, path) {
  return {
    ...
    children: Object.keys(module._children).map((moduleName) =>
      formatStoreForInspectorTree(module._children[moduleName], path + moduleName + '/'),
    ),
  };
}
